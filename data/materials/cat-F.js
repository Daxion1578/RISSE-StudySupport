    ...
    sections: [
      {
        ...
        body: `<p>ファイアウォールは、あらかじめ定めたルール（送信元/宛先IPアドレス、ポート番号など）に基づいて通信を許可・遮断する機器です。「必要な通信だけを通し、それ以外はすべて拒否する」という考え方（ホワイトリスト方式）が基本です。</p>`
      },
      {
...
<text x="320" y="315" font-family="sans-serif" font-size="13" fill="var(--ink)" text-anchor="middle">社内LAN（業務端末・内部サーバー）</text>
      ...
      {
        heading: `フィルタリングの方式`,
        body: `<p>フィルタリングには、パケットのヘッダー情報だけを見る<b>パケットフィルタリング型</b>と、通信の流れ（コネクションの状態）を管理してより高精度に判断する<b>ステートフルインスペクション型</b>があります。現在の多くのファイアウォールは後者です。</p>`
      },
      {
        heading: `設計のポイント`,
        body: `<p>DMZに置くのは、外部に公開する必要があるサーバー（Web・メール・DNSなど）だけにします。DMZのサーバーが社内LANへ通信を開始することは原則禁止し、万一DMZのサーバーが乗っ取られても社内へ侵入されないようにします。</p>`
      },
      {
        heading: `試験ではこう出る`,
        body: `<p>科目A-2では、DMZに配置すべきサーバーの組み合わせや、ファイアウォールの方式（パケットフィルタリング型・ステートフルインスペクション型）の特徴を問う問題が出ます。科目Bでは、ネットワーク構成図が示され、「どの通信を許可・拒否すべきか」「なぜこの構成では侵入が社内まで広がるか」を記述させる出題が見られます。</p>`
      }
    ],
    terms: [
  ...
  f2: {
    ...
    sections: [
      {
        heading: `3つの防御機器の違い`,
        body: `<p>IDS・IPS・WAFはいずれも不正な通信を見つける仕組みですが、見る対象と対応が異なります。</p>
<table>
...
<tr><td>WAF</td><td>Webアプリへの通信内容（SQLi/XSS等）</td><td>検知して遮断・通知する</td></tr>
      ...
      },
      {
        ...
        body: `<p>不正の見つけ方には、既知の攻撃パターン（<b>シグネチャ</b>）と一致するかを見る<b>シグネチャ型</b>と、普段と違う通信量やふるまいを見つける<b>アノマリ型</b>があります。シグネチャ型は未知の攻撃を見逃しやすく、アノマリ型は正常な通信を誤って攻撃と判定する<b>フォールスポジティブ（誤検知）</b>が起きやすいという弱点があります。</p>`
      },
      {
        heading: `配置と誤検知`,
        body: `<p>IDSは通信経路の外に置いて監視だけを行い、IPSは通信経路の途中（インライン）に置いて遮断まで行います。WAFはWebサーバーの手前に置きます。</p>`,
...
<text x="320" y="256" font-family="sans-serif" font-size="13" fill="var(--ink)" text-anchor="middle">Webサーバー・社内ネットワーク</text>
      ...
      {
        heading: `運用上の注意`,
        body: `<p>IPSやWAFは自動遮断するため、誤検知（フォールスポジティブ）が起きると正常な利用者の通信まで止めてしまいます。導入直後は検知のみのモードで様子を見てからルールを調整し、遮断モードへ切り替える運用が一般的です。</p>`
      },
      {
        heading: `試験ではこう出る`,
        body: `<p>科目A-2では、IDS/IPS/WAFの守備範囲の違いや、フォールスポジティブ・フォールスネガティブの意味を問う問題が出ます。科目Bでは、ログの一部が示され「これはどの機器で検知できるか」「誤検知を減らすにはどう設定すべきか」を答えさせる出題が見られます。</p>`
      }
    ],
    terms: [
  ...
  f3: {
    ...
    sections: [
      {
        heading: `VPNの仕組み`,
        body: `<p><b>VPN（Virtual Private Network）</b>は、インターネットのような公衆網の上に、暗号化された仮想的な専用回線を作る技術です。テレワークで社外から社内ネットワークに安全に接続する用途で広く使われ、暗号化にIPsecやTLSを使う方式が代表的です。</p>`
      },
      {
        heading: `従来型（境界防御）とゼロトラストの違い`,
        body: `<p>従来型のVPNは「社内ネットワークに入れた通信は信頼する」という<b>境界防御</b>の考え方が前提でした。しかしテレワークやクラウド利用が進み、「社内か社外か」で区別する意味が薄れています。</p>`,
...
<text x="535" y="208" font-family="sans-serif" font-size="13" fill="var(--ink)" text-anchor="middle">必要な範囲のみ最小権限で許可</text>
      ...
      {
        heading: `ゼロトラストの考え方`,
        body: `<p><b>ゼロトラスト</b>は「何も信頼せず、常に検証する」という考え方です。社内・社外を区別せず、アクセスのたびに利用者の認証情報・端末の状態（最新のパッチが当たっているか等）を確認し、必要なリソースだけに最小権限でアクセスを許可します。実装の一つに<b>ZTNA（Zero Trust Network Access）</b>があります。</p>`
      },
      {
        heading: `導入のポイント`,
        body: `<p>VPNは「入り口」を一つに絞れる利点がある一方、その入り口が破られると内部に広く侵入されるリスクがあります。ゼロトラストへの移行は一度に全社では難しいため、重要度の高いシステムから段階的にアクセス制御を厳格化していくのが現実的です。</p>`
      },
      {
        heading: `試験ではこう出る`,
        body: `<p>科目A-2では、VPNの暗号化方式やゼロトラストの基本概念を問う問題が出ます。科目Bでは、テレワーク環境の構成が示され、「境界防御の限界」や「ゼロトラストではどのようにアクセス制御すべきか」を説明させる出題が見られます。</p>`
      }
    ],
    terms: [
    ...
    sections: [
      {
        ...
        body: `<p>メールの送信元（Fromアドレス）は誰でも自由に書き換えられるため、実在する組織を装ったなりすましメール（フィッシングやビジネスメール詐欺）が容易に作れてしまいます。これを防ぐための送信ドメイン認証の仕組みが<b>SPF・DKIM・DMARC</b>です。科目Bで繰り返し出題される定番テーマです。</p>`
      },
      {
...
<tr><td>DMARC</td><td>SPF・DKIMの結果を踏まえ、失敗したメールを「隔離」「拒否」などどう扱うかをドメイン側が指定</td></tr>
      ...
      {
        heading: `検証の流れ`,
        body: `<p>受信側のメールサーバーは、次のような順序でなりすましを判定します。</p>`,
...
<text x="465" y="300" font-family="sans-serif" font-size="13" fill="var(--ink)" text-anchor="middle">認証成功 → 受信トレイへ配信</text>
      ...
      {
        heading: `設定のポイント`,
        body: `<p>SPFはDNSのTXTレコードに送信元サーバーの一覧を記載します。DKIMは送信サーバーが秘密鍵でメールに署名し、受信側は公開鍵（同じくDNSのTXTレコード）で検証します。DMARCも同様にDNSのTXTレコードでポリシー（<code>p=quarantine</code>や<code>p=reject</code>など）を宣言します。3つとも自ドメインのDNSレコードを正しく整備することが前提です。</p>`
      },
      {
        heading: `試験ではこう出る`,
        body: `<p>科目A-2では、SPF・DKIM・DMARCそれぞれが確認する内容の組み合わせを問う問題が定番です。科目Bでは、ドメイン名変更やメールシステム更改を題材に、DNSレコードの設定内容や「なぜなりすましメールを検知できたか／できなかったか」を記述させる出題が実際にありました（R6秋）。</p>`
      }
    ],
    terms: [
    ...
    sections: [
      {
        ...
        body: `<p>DNSは名前解決という重要な役割を担う一方、応答の正当性を検証する仕組みが本来備わっていません。偽の応答を信じ込ませることができれば、利用者を偽サイトへ誘導したり通信を盗聴したりできてしまいます。</p>`
      },
      {
        heading: `キャッシュポイズニングの手順`,
        body: `<p>代表的な攻撃が<b>DNSキャッシュポイズニング</b>です。キャッシュDNSサーバー（利用者の問い合わせを受けて答えを一時保存するサーバー）に偽の情報を覚え込ませます。</p>`,
        svg: `<svg viewBox="0 0 640 360" width="100%" style="max-width:640px;height:auto">
<rect x="70" y="16" width="500" height="44" rx="6" fill="none" stroke="var(--grid)"/>
...
<text x="320" y="315" font-family="sans-serif" font-size="13" fill="var(--ink)" text-anchor="middle">⑤ 利用者が偽サイトへ誘導される（対策: DNSSEC等）</text>
      ...
      {
        heading: `DNSSECによる対策`,
        body: `<p>対策として、DNSの応答に電子署名を付けて改ざん・偽装を検知できるようにする<b>DNSSEC</b>があります。また、問い合わせのポート番号や識別子(トランザクションID)をランダム化し、攻撃者が偽応答を当てにくくする対策も基本的な防御として有効です。</p>`
      },
      {
        heading: `その他の脅威（サブドメインテイクオーバー）`,
        body: `<p>もう一つの脅威が<b>サブドメインテイクオーバー</b>です。クラウドサービスなどを紐付けたサブドメイン（例: shop.example.jp）のDNS設定を消し忘れたまま、紐付け先のサービス契約を解約すると、そのサブドメイン名を他人が乗っ取り、正規ドメインの一部として悪意あるコンテンツを配信できてしまいます。使わなくなったDNSレコードは速やかに削除することが対策です。</p>`
      },
      {
        heading: `試験ではこう出る`,
        body: `<p>科目A-2では、DNSSECの目的やキャッシュポイズニングの成立条件を問う問題が出ます。科目Bでは、ドメイン名の変更や移管を題材に「DNS設定の切り替え時にどのようなリスクがあるか」「サブドメインテイクオーバーをどう防ぐか」を記述させる出題が実際にありました（R6秋）。</p>`
      }
    ],
    terms: [
    ...
    sections: [
      {
        ...
        body: `<p>プロキシは通信を代理で中継する仕組みですが、置く場所によって役割が異なります。</p>`,
...
<text x="540" y="229" font-family="sans-serif" font-size="13" fill="var(--ink)" text-anchor="middle">社内のWebサーバー</text>
      ...
      {
        heading: `ログに残る情報`,
        body: `<p>プロキシやWebサーバーのログには、アクセス日時、送信元IPアドレス、リクエストされたURL、HTTPメソッド、応答ステータスコード、User-Agent（利用ブラウザの情報）などが記録されます。これらは平常時の記録であると同時に、インシデント発生時の重要な調査材料になります。</p>`
      },
      {
        heading: `ログから攻撃を読む`,
        body: `<p>例えば、同一IPアドレスから短時間に大量の401（認証失敗）が記録されていれば総当たり攻撃、URLに<code>' OR '1'='1</code>のような文字列が含まれていればSQLインジェクションの試行が疑われます。ログを時系列に並べ、通常と異なるパターン（普段アクセスしない時間帯・国、大量の同一パターンのリクエストなど）を見つけることが検知の基本です。</p>`
      },
      {
        heading: `運用のポイント`,
        body: `<p>ログは改ざんされると調査ができなくなるため、ログ専用サーバーに転送し、書き込んだサーバー側では変更できないようにする、保存期間を定めて必要な期間残す、といった管理が必要です。</p>`
      },
      {
        heading: `試験ではこう出る`,
        body: `<p>科目A-2では、フォワードプロキシとリバースプロキシの役割の違いを問う問題が出ます。科目Bでは、実際のアクセスログの抜粋が提示され、「どの行が攻撃の痕跡か」「攻撃の種類は何か」を読み取らせる出題が頻出です。ログの読み方に慣れておくことが得点につながります。</p>`
      }
    ],
    terms: [
    ...
    sections: [
      {
        ...
        body: `<p>無線LANの暗号化方式は、脆弱性が見つかるたびに置き換えられてきました。<b>WEP</b>は既に解読方法が確立しており使用してはいけません。<b>WPA2</b>はAESベースの暗号化で長く使われてきましたが、事前共有鍵（パスワード）を推測される弱点があり、後継の<b>WPA3</b>ではより安全な鍵交換方式（SAE）が採用されています。</p>`
      },
      {
        heading: `エンタープライズ認証（WPA2/3-Enterprise）`,
        body: `<p>家庭用の多くは全員が同じパスワード（事前共有鍵）を使う<b>パーソナルモード</b>ですが、企業では利用者ごとにIDとパスワードで認証する<b>エンタープライズモード</b>（WPA2/WPA3-Enterprise）を使い、IEEE802.1XとRADIUSサーバーによる個別認証を行います。これにより、退職者のアクセスだけを個別に無効化できます。</p>`
      },
      {
        heading: `偽アクセスポイントのリスクと対策`,
        body: `<p>攻撃者が正規のアクセスポイントに似せた名前（SSID）の<b>偽アクセスポイント（Evil Twin）</b>を用意し、利用者を誤って接続させて通信を盗聴する手口があります。対策として、企業では利用者証明書によるエンタープライズ認証を使う、公衆Wi-Fi利用時は必ずVPNを併用する、といった対策が有効です。</p>`
      },
      {
        heading: `試験ではこう出る`,
        body: `<p>科目A-2では、WEP/WPA2/WPA3の暗号化方式の違いや、パーソナルモードとエンタープライズモードの違いを問う問題が出ます。科目Bでは、社外での無線LAN利用を題材に、偽アクセスポイントのリスクや、社員に周知すべき対策を記述させる出題が見られます。</p>`
      }
    ],
    terms: [
