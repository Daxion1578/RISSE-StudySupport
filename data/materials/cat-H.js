...
Object.assign(window.MATERIALS, {
  h1: {
    ...
    sections: [
      {
        heading: "不正アクセス禁止法",
        body: '<p><b>不正アクセス禁止法</b>は、他人のID・パスワードを無断で使ってログインしたり、認証を回避してシステムに侵入したりする行為を処罰する法律です。実際に被害が発生していなくても、不正にログインした時点で成立します。また、他人のID・パスワードを不正に取得・保管・提供する行為（フィッシングでの取得なども含む）も規制対象です。例えば、退職した元従業員が在職中に使っていたIDを使い続けて社内システムにログインした場合も、権限が失効している以上この法律の対象になり得ます。</p>'
      },
      {
        heading: "刑法によるサイバー犯罪の処罰",
        body: '<p>刑法には、ウイルスを作成・提供する行為を処罰する「不正指令電磁的記録に関する罪」や、サーバに大量アクセスを行い機能を停止させる行為を処罰する「電子計算機損壊等業務妨害罪」などの規定があります。不正アクセス禁止法が「侵入（ログイン）」を対象とするのに対し、刑法のこれらの罪は「破壊・妨害・ウイルスの作成」といった行為を対象とする点が異なります。DDoS攻撃でサーバを停止させる行為は、不正アクセス禁止法ではなく主に電子計算機損壊等業務妨害罪の対象となる点も、行為と法律の対応関係として押さえておきましょう。</p>'
      },
      {
        heading: "どの法律が誰に適用されるか",
        body: '<p>サイバーセキュリティ基本法は、国や重要インフラ事業者に対しセキュリティ対策の責務を定める法律で、個人の犯罪行為を直接処罰するものではありません。電子署名法は、電子契約に手書きの署名や押印と同等の法的効力を与えるための法律です。試験では「この行為はどの法律に違反するか」「違反した場合、行為者はどうなるか（懲役・罰金）」という形で、行為と法律の対応関係が問われます。3つの法律は目的も処罰対象も異なるため「セキュリティに関する法律だから全部同じ」と混同せず、それぞれ何を防ぐための法律かを整理しておくことが得点につながります。</p>',
      ...
      {
        heading: "試験ではこう出る",
        body: '<p>科目Aでは、各法律の対象行為や罰則の有無を問う四択が出題されます。科目Bでは、攻撃者の行為が時系列で示され「この行為は不正アクセス禁止法・刑法のどちらに、どの条項に該当する可能性があるか」を記述させる問題が出ます。「悪いことをした」で終わらせず、どの法律のどの行為類型に当たるかまで具体的に答える必要があります。「ログインに成功したか」「システムを壊したか・止めたか」という行為の性質の違いに着目すると、適用法令を切り分けやすくなります。複数の法律が同時に成立する場合もあるため、択一で答えるより「該当する可能性がある法律を挙げ、その理由を書く」姿勢が記述では評価されます。</p>'
      }
    ],
    terms: [
  ...
  h2: {
    ...
    sections: [
      {
        heading: "個人情報とは何か",
        body: '<p>個人情報保護法における「個人情報」とは、氏名や生年月日など特定の個人を識別できる情報のことです。中でも人種・病歴・犯罪歴などは「要配慮個人情報」として、取得・利用に本人の同意が必要となるなど、より厳しく扱われます。氏名などを削除・置換して特定の個人を識別できないよう加工した「仮名加工情報」「匿名加工情報」は、一定の条件のもとで通常の個人情報より柔軟に扱うことが認められています。例えば会員IDと氏名の対応表を別管理にして氏名を削除しただけの「仮名加工情報」は、対応表と突き合わせれば元に戻せるため、匿名加工情報より慎重な取り扱いが求められます。</p>'
      },
      {
        heading: "漏えい時の報告義務",
      ...
      {
        heading: "安全管理措置の義務",
        body: '<p>個人情報取扱事業者には、取り扱う個人データの漏えい・滅失・毀損を防ぐために必要かつ適切な「安全管理措置」を講じる義務があります。これは組織的・人的・物理的・技術的の4つの側面から求められ、アクセス制御や従業員教育、入退室管理などがこれにあたります。委託先に個人データの取り扱いを委託する場合は、委託先に対する必要かつ適切な監督義務も負います。委託先で漏えいが起きた場合でも、委託元が監督義務を果たしていなかったと判断されれば、委託元自身の責任も問われる点に注意が必要です。</p>'
      },
      {
        heading: "試験ではこう出る",
        body: '<p>科目Aでは、要配慮個人情報の具体例や、報告義務が発生する条件、報告期限の日数を問う四択が出題されます。科目Bでは、漏えい事故のシナリオが示され「この事案は個人情報保護委員会への報告義務が発生する条件を満たすか、理由とともに述べよ」「委託先の監督義務として何を行うべきだったか」を記述させる問題が頻出です。「義務があるかないか」だけでなく、条文上の該当理由を具体的に書けるようにしておきましょう。件数や情報の種類（要配慮個人情報が含まれるか等）を問題文から正確に読み取り、報告義務の該当条件と照らし合わせる練習をしておくことが得点につながります。</p>'
      }
    ],
    terms: [
    ...
    sections: [
      {
        ...
        body: '<p>セキュリティ対策の「型」を示す基準・ガイドラインは複数存在し、それぞれ目的や対象範囲が異なります。すべてを暗記するより、「誰のため・何のための基準か」という軸で整理して覚えると混同しにくくなります。自社で一からセキュリティ対策を設計するのは大変な作業ですが、こうした基準を土台にすれば、世界中の専門家の知見を効率よく取り入れることができます。</p>'
      },
      {
      ...
      },
      {
        ...
        body: '<p>NIST CSFのように「考え方の枠組み」を示す基準と、CIS ControlsやPCI DSSのように「具体的にやるべきこと」を列挙した基準は性格が異なります。前者は自社の状況に合わせて対策の全体像を設計する際に、後者は実際に何を導入・運用すべきかを決める際に使われます。ISMAPのように特定の調達・分野に限定して使われる基準もあり、対象範囲の違いも押さえておく必要があります。例えば「まず自社のセキュリティの現状を俯瞰したい」ならNIST CSF、「具体的に何を導入すればよいか知りたい」ならCIS Controlsを参照する、というように目的に応じて使い分けます。</p>'
      },
      {
        heading: "試験ではこう出る",
    ...
    sections: [
      {
        ...
        body: '<p>情報処理安全確保支援士（登録セキスペ）は、試験合格後に登録することで名乗れる国家資格です。試験合格そのものではなく、登録して初めて「情報処理安全確保支援士」を名乗ることができ、登録後は3年ごとの更新と、継続的な講習の受講が義務付けられています。試験合格の効力自体には有効期限がなく、合格したのに登録しない、あるいは一度登録した後に登録を抹消するという選択も可能です。登録しない場合、試験合格者ではありますが「情報処理安全確保支援士」を名乗ることはできません。</p>'
      },
      {
        heading: "支援士に課される義務",
        body: '<p>支援士には、業務上知り得た秘密を漏らしてはならない「秘密保持義務」や、資格の信用を傷つける行為をしてはならない「信用失墜行為の禁止」などが法律上の義務として課されています。これらは資格を持つ専門家として、顧客や社会からの信頼を守るための最低限のルールです。例えば、業務で担当した企業の脆弱性情報をSNSで話題として発信してしまう行為は、たとえ悪意がなくても秘密保持義務に抵触するおそれがあります。</p>'
      },
      {
        heading: "違反した場合",
        body: '<p>秘密保持義務などに違反した場合、登録の取消しや名称の使用停止といった行政処分の対象となるほか、違反の内容によっては罰則（罰金など）が科されることもあります。資格を持っているだけで安泰なのではなく、義務を守り続けることが資格維持の前提です。行政処分を受けると、その事実が公表される場合もあり、専門家としての信用そのものを失うことになります。</p>'
      },
      {
        heading: "登録は任意、更新には講習が必要",
        body: '<p>試験に合格しても登録は義務ではなく、登録するかどうかは本人の判断です。登録すると「情報処理安全確保支援士」を名乗って業務にあたれる一方、3年ごとの更新のたびに実践講習などを受講する必要があり、費用と時間の負担も生じます。実務でこの資格を必要としない人は、登録せずに合格の実績だけを履歴書等でアピールするという選択も現実的です。契約上「登録セキスペの在籍」が求められる業務に関わる予定がある場合は、あらかじめ登録の要否を確認しておくとよいでしょう。</p>'
      },
      {
        heading: "試験ではこう出る",
        body: '<p>科目Aでは、支援士の登録更新の周期（3年）や、秘密保持義務・信用失墜行為の禁止といった義務の内容、違反時の扱いを問う四択が出題されます。科目Bでの直接的な出題は多くありませんが、他の法規（個人情報保護法など）と絡めて「専門家としてどう行動すべきだったか」という形で間接的に問われることがあります。制度の細部を丸暗記するより、「なぜこの義務があるのか（信頼を守るため）」という趣旨から理解しておくと、初見の四択にも対応しやすくなります。</p>'
      }
    ],
    terms: [
      ...
      { term: "登録の任意性", desc: "試験合格後に登録するかどうかは本人の自由な選択に委ねられている。" }
    ]
  },
  h5: {
    unitId: "h5",
    ...
    sections: [
      {
        heading: "著作権とソフトウェア",
        body: '<p>プログラムは著作物として著作権法で保護されており、無断で複製・改変・配布すると著作権侵害になります。一方でOSS（オープンソースソフトウェア）は、ライセンス（利用許諾条件）に従うことを条件に、複製・改変・再配布が認められています。ライセンスの条件を守らずに利用すると、OSSであっても著作権侵害になり得る点に注意が必要です。例えば、改変したソースコードの公開を義務付けるライセンスのOSSを自社製品に組み込んだのに、その部分を非公開のまま販売すると、ライセンス違反として著作権侵害を問われる可能性があります。「無料で公開されている＝自由に何をしてもよい」ではない点は、OSSを利用する開発者が特に誤解しやすいポイントです。</p>'
      },
      {
        heading: "労働形態と指揮命令権",
        body: '<p>システム開発の委託形態には「請負」と「派遣」があります。請負は成果物の完成を約束する契約で、委託先の労働者への指揮命令は委託先の管理者が行います。派遣は労働者そのものを提供する契約で、派遣先が労働者に直接指揮命令を行うことができます。請負契約でありながら委託元が現場の作業者に直接指示を出すと「偽装請負」として労働関連法規に違反する可能性があります。多重下請けの現場では契約形態と実際の指示系統がずれやすく、意図せず偽装請負の状態になっていることもあるため注意が必要です。</p>'
      },
      {
        heading: "下請法による保護",
        body: '<p>下請法（下請代金支払遅延等防止法）は、発注者（親事業者）が下請事業者に対して代金の支払いを不当に遅らせたり、一方的に減額したりすることを禁止する法律です。IT業界では、システム開発を多重下請け構造で行うことが多く、立場の弱い下請事業者を保護する目的でこの法律が適用されます。発注内容を書面で明確にせず、後から一方的に仕様変更や代金の減額を求める行為も、下請法違反として問題になり得ます。発注書面の交付義務や、代金の支払期日を発注から60日以内とする規定など、具体的なルールが定められています。</p>'
      },
      {
        heading: "試験ではこう出る",
        body: '<p>科目Aでは、著作権法上OSSライセンスに違反する行為や、請負と派遣における指揮命令権の違い、偽装請負の定義、下請法が保護する対象を問う四択が出題されます。科目Bでの出題頻度は高くありませんが、委託先管理と絡めて契約形態の違いが問われることがあります。頻出ポイントに絞って押さえておけば十分です。「誰が誰に指示を出しているか」という関係図を頭の中で描きながら問題文を読むと、契約形態の判別がしやすくなります。専門的な法律用語をすべて覚える必要はなく、試験で問われる範囲に絞って理解しておけば十分対応できます。</p>'
      }
    ],
    terms: [
